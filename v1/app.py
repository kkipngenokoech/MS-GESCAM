...
Dash dashboard for MS-GESCAM Classroom Analytics
"""
...
# Use the Dash ngrok URL from 'ngrok start --all'
NGROK_URL = "https://660b-41-186-78-169.ngrok-free.app"
print(f" * Dash ngrok tunnel: {NGROK_URL}")

# API Endpoints (using Flask API's ngrok URL)
FLASK_API_URL = "https://ef1d-41-186-78-169.ngrok-free.app"
API_UPLOAD_FRAME = f"{FLASK_API_URL}/upload_frame"
API_GET_FRAME = f"{FLASK_API_URL}/get_frame"
API_ANALYZE_FRAME = f"{FLASK_API_URL}/analyze_frame"
API_ANALYZE_TEMPORAL = f"{FLASK_API_URL}/analyze_temporal"
API_VISUALIZE_FRAME = f"{FLASK_API_URL}/visualize_frame"
...
def generate_trend_figure(upload_id, frame_count):
    ...
    try:
        ...
        for frame_idx in range(frame_count):
            x_data = [frame_idx]
            y_data = [frames_data[frame_idx]['mean_attention']]
            ...
            step = dict(
                ...
                args=[
                    {
                        "x": [x_data] + [person_x_data] * len(person_ids),
                        "y": [y_data] + [[score] for score in person_y_data]
                    },
...
def handle_upload(upload_contents, upload_filename, mode):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            interval = 3600000 if frame_count == 1 else 1000
...
if __name__ == '__main__':
    try:
        subprocess.run(["pip", "install", "dash", "plotly", "requests", "pyngrok"], check=True)
